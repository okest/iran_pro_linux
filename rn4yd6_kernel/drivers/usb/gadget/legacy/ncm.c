...
/*-------------------------------------------------------------------------*/
static struct usb_composite_dev *mycdev=NULL;
static int __init gncm_bind(struct usb_composite_dev *cdev)
{
	...
	if(!strcmp("ci_hdrc_sirf.0", gadget->name))
	{
		gcnum = 0x33;
...
static int __init ncm_init(void)
{
	...
	res=usb_composite_probe(&ncm_driver);
	usb_gadget_disconnect(mycdev->gadget);
	usb_gadget_connect(mycdev->gadget);	
...
static void __exit ncm_cleanup(void)
{
	//usb_gadget_disconnect(mycdev->gadget);
	
	usb_composite_unregister(&ncm_driver);
/*
	device_destroy(carplay_class, MKDEV(0, carplay_dev->index));
	class_destroy(carplay_class);
	kfree(carplay_dev);
	carplay_dev = NULL;
*/
}
module_exit(ncm_cleanup);
