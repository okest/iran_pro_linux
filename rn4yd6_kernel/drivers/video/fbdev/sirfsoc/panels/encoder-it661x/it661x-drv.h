...
#ifndef _IT6611_DRV_H_
#define _IT6611_DRV_H_
...
#define IT6611_I2C_SLAVE_ADDR 0x98
...
#define SWITCH_HDMITX_BANK(x)	it661x_write_i2c_byte(0x0f, (x) & 1)
#define ENABLE_NULL_PKT()	it661x_write_i2c_byte(REG_NULL_CTRL,\
					B_ENABLE_PKT | B_REPEAT_PKT)

#define ENABLE_ACP_PKT()	it661x_write_i2c_byte(REG_ACP_CTRL,\
					B_ENABLE_PKT | B_REPEAT_PKT)

#define ENABLE_ISRC1_PKT()	it661x_write_i2c_byte(REG_ISRC1_CTRL,\
					B_ENABLE_PKT | B_REPEAT_PKT)

#define ENABLE_ISRC2_PKT()	it661x_write_i2c_byte(REG_ISRC2_CTRL,\
					B_ENABLE_PKT | B_REPEAT_PKT)

#define ENABLE_AVI_INFO_FRM_PKT()\
			it661x_write_i2c_byte(REG_AVI_INFOFRM_CTRL,\
				B_ENABLE_PKT | B_REPEAT_PKT)

#define DISABLE_NULL_PKT()\
		do {\
			SWITCH_HDMITX_BANK(0);\
			it661x_write_i2c_byte(REG_NULL_CTRL, 0);\
		} while (0)

#define DISABLE_ACP_PKT()\
		do {\
			SWITCH_HDMITX_BANK(0);\
			it661x_write_i2c_byte(REG_ACP_CTRL, 0);\
		} while (0)

#define DISABLE_ISRC1_PKT()\
		do {\
			SWITCH_HDMITX_BANK(0);\
			it661x_write_i2c_byte(REG_ISRC1_CTRL, 0);\
		} while (0)

#define DISABLE_ISRC2_PKT()\
		do {\
			SWITCH_HDMITX_BANK(0);\
			it661x_write_i2c_byte(REG_ISRC2_CTRL, 0);\
		} while (0)

#define DISABLE_AVI_INFO_FRM_PKT()\
		do {\
			SWITCH_HDMITX_BANK(0);\
			it661x_write_i2c_byte(REG_AVI_INFOFRM_CTRL, 0);\
		} while (0)

#define ENTER_FUNC(x) /* (printk(x)) */
...
bool it661x_read_i2c_byteN(unsigned char reg_addr, unsigned char *pdata, int n);
bool it661x_write_i2c_byteN(unsigned char reg_addr,
					unsigned char *pdata, int n);

#define it661x_or_reg_byte(reg, ormask) it661x_write_i2c_byte(reg,\
			(it661x_read_i2c_byte(reg) | (ormask)))

#define it661x_and_reg_byte(reg, andmask) it661x_write_i2c_byte(reg,\
			(it661x_read_i2c_byte(reg) & (andmask)))

#define it661x_set_reg_byte(reg, andmask, ormask) it661x_write_i2c_byte(reg,\
			((it661x_read_i2c_byte(reg) & (andmask))|(ormask)))
