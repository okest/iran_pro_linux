...
static int i_brightness_value = 0;                 
static int __init __brightness_setup(char *str)
{
    int i_mm;
    for (i_mm = 0; i_mm < strlen(str); i_mm++)
    {
        ( i_brightness_value ) = ((i_brightness_value) * 10) + (str[i_mm] - '0');
    }
 
    return 0;
}
__setup("brightness=", __brightness_setup);
...
static int pwm_backlight_parse_dt(struct device *dev,struct platform_pwm_backlight_data *data)
{
	...
	if (data->max_brightness > 0) {
		...
		if(i_brightness_value>0 && i_brightness_value <17)
		{
			data->dft_brightness = i_brightness_value;		
			pr_info("[%s][%d]   i_brightness_value = %d \n", __FUNCTION__, __LINE__, i_brightness_value ); 
		}
		else
		{
