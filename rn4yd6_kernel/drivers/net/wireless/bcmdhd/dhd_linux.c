 ...
 * example nvram_path[MOD_PARAM_PATHLEN]="/projects/wlan/nvram.txt"
 */
char firmware_path[MOD_PARAM_PATHLEN]="/etc/firmware/fw_bcm40181a2.bin";
char nvram_path[MOD_PARAM_PATHLEN]="/etc/firmware/nvram_ap6181.txt";
char config_path[MOD_PARAM_PATHLEN];

/* backup buffer for firmware and nvram path */
char fw_bak_path[MOD_PARAM_PATHLEN]="/etc/firmware/fw_bcm40181a2.bin";
char nv_bak_path[MOD_PARAM_PATHLEN]="/etc/firmware/nvram_ap6181.txt";	
...
//	setbit(eventmask, WLC_E_SCAN_COMPLETE); // terence 20150628: remove redundant event
	...
	if (disable_proptx ||
		...
		// terence 20170115: PROP_TXSTATUS_VSDB only defined for 4339
		((dhd->conf->chip == BCM4335_CHIP_ID || dhd->conf->chip == BCM4339_CHIP_ID) &&
		dhd->op_mode != DHD_FLAG_HOSTAP_MODE &&
		 dhd->op_mode != DHD_FLAG_IBSS_MODE) ||
#endif /* PROP_TXSTATUS_VSDB */
		FALSE) {
...
int gpio_wl_reg_on = -1;    // WL_HOST_WAKE is output pin of WLAN module
int gpio_wl_host_wake = -1; // WL_HOST_WAKE is output pin of WLAN module
int gpio_wl_pwr = -1;

#define WIFI_NAME  "bcm_wlan"

static int wifi_probe(struct platform_device *pdev)
{
	struct device_node *np = pdev->dev.of_node;
	
	gpio_wl_pwr    = of_get_named_gpio( np , "wifi,gpio_wl_pwr", 0);
	gpio_wl_reg_on = of_get_named_gpio( np , "wifi,gpio_wl_reg_on", 0);
	gpio_wl_host_wake = of_get_named_gpio( np , "wifi,gpio_wl_host_wake", 0);
...
static struct of_device_id bcm_wifi_match_tbl[] = {
	{ .compatible = "wifi,bcm_wlan", },
	{ /* end */ }
};
...
static struct platform_driver bcm_wifi_driver = {
	.driver		= {
		.name = WIFI_NAME,
		.of_match_table = bcm_wifi_match_tbl,
	},
	.probe = wifi_probe,
	.remove = wifi_remove,
};
...
{
	dhd_module_cleanup();
	platform_driver_unregister(&bcm_wifi_driver);
...
static int __init dhd_module_init(void)
{
	...
	err = platform_driver_register(&bcm_wifi_driver);
	if (err) {
		printk(KERN_ALERT"[%s][%d] platform_driver_register failed \n", __FUNCTION__,__LINE__);
		return err;
	}	
	...
	pr_info("firmware_path =%s \n", firmware_path);
	pr_info("nvram_path    =%s \n", nvram_path);
	pr_info("fw_bak_path   =%s \n", fw_bak_path);			
	pr_info("nv_bak_path   =%s \n", nv_bak_path);
	...
	if (err)
	{
		...
		{
			errcode_repo(ENUM_HAL_CODE_WIFI,ENUM_HAL_VALUE_WIFI_INIT_FAIL);   //存储启动之后的错误码
			printk(KERN_EMERG "LINE = %d FUNC = %s repo a code ！！！\n",__LINE__, __func__);
		}
	}	
	pr_info("[WIFI]********** err=%d  ************END\n", err);
	return err;
}	
