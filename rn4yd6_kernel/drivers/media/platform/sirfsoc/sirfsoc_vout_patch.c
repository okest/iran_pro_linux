...
//zjc
static struct file *carbit_file = NULL;
/* File operations */
static int sirfsoc_vout_open(struct file *file)
{
	...
	printk(KERN_ALERT"zjc\n[%s][%d][out] process[%s] id=%d  pid=%d tgid=%d \n",__FUNCTION__,__LINE__,pcurrent->comm,id,pcurrent->pid,pcurrent->tgid);	
	//printk(KERN_EMERG"zjc-------> open %p id = %d  f_count = %d\n",vout,id,(file->f_count));	
	
	v4l2_dbg(1, debug, v4l2_dev, "Exit %s\n", __func__);
		
//add for app_carbit	zjc	
#if 1
	
	if( ( id == 2 )  && ( strcmp( current->comm ,"app_carbit") == 0 ) ){
		printk(KERN_EMERG "[%s][%d] ********** %s is using layer2  id=%d \n",__func__,__LINE__,current->comm,id);
		carbit_file = file;
	}
#endif 
	return 0;
...
//zjc
#if 1
void carbit_release(void)
{
	struct sirfsoc_vout_device *vout = carbit_file->private_data;
	struct v4l2_device *v4l2_dev;
	struct sirfsoc_vdss_layer *l;
	enum vdss_layer id;
	static struct task_struct *pcurrent;
	
	printk(KERN_EMERG"zjc------->proc  close carbit layer2 \n");	
	if (vout == NULL)
		return -ENODEV;

	id = vout->vd->index % SIRFSOC_MAX_VOUT_ON_EACH_DISPLAY + 1;
	pcurrent = get_current();
	printk(KERN_ALERT"[%s][%d][in] process[%s] id=%d  pid=%d tgid=%d \n",__FUNCTION__,__LINE__,pcurrent->comm,id,pcurrent->pid,pcurrent->tgid);	
...
static struct proc_dir_entry *carbit_proc = NULL;

static ssize_t carbit_read_proc(struct file *file, const char __user *buffer, size_t count, loff_t *off);
static ssize_t carbit_write_proc(struct file *file, const char __user *buffer, size_t count, loff_t *off);

static const struct file_operations carbit_proc_ops = {
    .owner = THIS_MODULE,
    .read = carbit_read_proc,
    .write = carbit_write_proc,
};
...
static ssize_t carbit_write_proc(struct file *file, const char __user *buffer, size_t count, loff_t *off)
{
	...
	{	 
		carbit_release();
		printk(KERN_EMERG " close carbit layer 2 %d  %s  \n",__LINE__, recv);		
	}
	return count;
}
#endif 
...
static int sirfsoc_vout_release(struct file *file)
{
	...
	id = vout->vd->index % SIRFSOC_MAX_VOUT_ON_EACH_DISPLAY + 1;
	pcurrent = get_current();
	printk(KERN_ALERT"zjc[%s][%d][in] process[%s] id=%d  pid=%d tgid=%d \n",__FUNCTION__,__LINE__,pcurrent->comm,id,pcurrent->pid,pcurrent->tgid);	
...
static int sirfsoc_vout_probe(struct platform_device *pdev)
{
	...
	//zjc	
	carbit_proc = proc_create("carbit", 0666, NULL, &carbit_proc_ops);
