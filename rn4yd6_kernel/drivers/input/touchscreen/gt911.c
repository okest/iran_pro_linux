...
static struct proc_dir_entry *gt9xx_enable_irq = NULL;
static ssize_t gt91xx_enable_read_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t gt91xx_enable_write_proc(struct file *, const char __user *, size_t, loff_t *);
struct goodix_ts_data *ts_all;
...
int  clear_gt9xx_irq(void);
int  enable_gt9xx_irq(void);

static DEFINE_MUTEX(gt9xxmutex);
...
static const struct file_operations enable_proc_ops = {
    .owner = THIS_MODULE,
    .read = gt91xx_enable_read_proc,
    .write = gt91xx_enable_write_proc,
};

static ssize_t gt91xx_enable_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	printk(KERN_EMERG "%p\n",ts_all);
	return 0;
}
int TS_ACC_FLAG = 0;
static ssize_t gt91xx_enable_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
	int ret;
	char recv[3];
	...
	{	 
		if(TS_ACC_FLAG == 1)
        {
			...
			clear_gt9xx_irq();			
			memset(point_data,0,2 + 1 + 8 * GTP_MAX_TOUCH + 1);
			point_data[0]=(GTP_READ_COOR_ADDR >> 8);
			point_data[1]= (GTP_READ_COOR_ADDR & 0xFF);		
		
			ret = gtp_i2c_write(i2c_connect_client, point_data, 2 + 1 + 8 * GTP_MAX_TOUCH + 1);	
			if (ret < 0)
			{
				printk(KERN_EMERG"[%d][gtp_i2c_write] I2C transfer error  %d\n",__LINE__, ret);
				return 0;
			}
	
			enable_irq(ts_all->client->irq);
			printk(KERN_EMERG"[%d][ACC ON]  enable_irq is %s  \n",__LINE__, recv);				
		}
	}
...
int i2c_read_buff_ds927(struct i2c_client *client, char *buf ,int len) 
{ 
    struct i2c_adapter *adap=client->adapter;  
    struct i2c_msg msg;                        
    int ret; 
 
    msg.addr = DS927_I2C_DEVICE_ADDRESS;                   
...
int i2c_write_buff_ds927(struct i2c_client *client,u8 *buf,s32 len)
{
    int ret=-1; 
	struct i2c_msg msgs;
	
	msgs.addr  = DS927_I2C_DEVICE_ADDRESS;
	msgs.flags = !I2C_M_RD;
	msgs.len   = len ;
	msgs.buf   = buf;
...
static unsigned char read_reg_ds927(struct i2c_client *client, unsigned char reg,s32 len) 
{ 
    unsigned char buf; 
	int s32Ret = -1;
	
	s32Ret =i2c_write_buff_ds927(i2c_connect_client,&reg, 1);
	if( s32Ret <0 )
	{
		printk(KERN_ALERT"[%s][%d]  i2c_write_buff_ds927 test1\n",__func__,__LINE__);
		s32Ret =i2c_write_buff_ds927(i2c_connect_client,&reg, 1);
		if( s32Ret <0 )
		{
			printk(KERN_ALERT"[%s][%d]  i2c_write_buff_ds927 test2\n",__func__,__LINE__);
			s32Ret =i2c_write_buff_ds927(i2c_connect_client,&reg, 1);
			if( s32Ret <0 )
			{
				printk(KERN_ALERT"[%s][%d]  i2c_write_buff_ds927 test3\n",__func__,__LINE__);
				return -1;
			}
		}
	}

    s32Ret = i2c_read_buff_ds927(i2c_connect_client, &buf, 1);  
	if( s32Ret <0 )
	{
		printk(KERN_ALERT"[%s][%d]  i2c_read_buff_ds927 failed1 \n",__func__,__LINE__);
		s32Ret = i2c_read_buff_ds927(i2c_connect_client, &buf, 1);  
		if( s32Ret < 0 )
		{
			printk(KERN_ALERT"[%s][%d]  i2c_read_buff_ds927 failed2 \n",__func__,__LINE__);
			return -1;
		}
	}
...
//==============================================================================
int i2c_read_buff_ds302(struct i2c_client *client, char *buf ,int len) 
{ 
    struct i2c_adapter *adap=client->adapter;  
    struct i2c_msg msg;                        
    int ret; 
 
    msg.addr = DS302_I2C_DEVICE_ADDRESS;                   
    msg.flags = client->flags & I2C_M_TEN;     
    msg.flags |= I2C_M_RD;                     
    msg.len = len;                              
    msg.buf = buf; 
...
int i2c_write_buff_ds302(struct i2c_client *client,u8 *buf,s32 len)
{
    int ret = -1; 
	struct i2c_msg msgs;

	msgs.flags = !I2C_M_RD;
	msgs.addr  = DS302_I2C_DEVICE_ADDRESS;
	msgs.len   = len ;
	msgs.buf   = buf;
	ret = i2c_transfer(client->adapter, &msgs, 1);
...
static unsigned char read_reg_ds302(struct i2c_client *client, unsigned char reg,s32 len) 
{ 
    unsigned char buf; 
	int s32Ret = -1;

	s32Ret =i2c_write_buff_ds302(i2c_connect_client,&reg, 1);
	if( s32Ret <0 )
	{
		printk(KERN_ALERT"[%s][%d]  i2c_write_buff_ds302 test1 \n",__func__,__LINE__);
		s32Ret =i2c_write_buff_ds302(i2c_connect_client,&reg, 1);
		if( s32Ret <0 )
		{
			printk(KERN_ALERT"[%s][%d]  i2c_write_buff_ds302 test2 \n",__func__,__LINE__);
			return -1;
		}
	}

    s32Ret = i2c_read_buff_ds302(i2c_connect_client, &buf, 1);  
	if( s32Ret <0 )
	{
		printk(KERN_ALERT"[%s][%d]  i2c_read_buff_ds302 failed1 \n",__func__,__LINE__);
		s32Ret = i2c_read_buff_ds302(i2c_connect_client, &buf, 1);  
		if( s32Ret <0 )
		{
			printk(KERN_ALERT"[%s][%d]  i2c_read_buff_ds302 failed2 \n",__func__,__LINE__);
			return -1;
		}
	}
    return  buf; 
...
int  GTP_RST_OUTPUT(int pin,int level)//REG6
{
	u8	u8Data[2]={0x20,0x90}; 
	s32 s32Ret = -1;
	
	if(level)
	{
		u8Data[1] = 0x90 ;
		s32Ret =i2c_write_buff_ds302(i2c_connect_client,u8Data, 2);
		if( s32Ret <0 )
		{
			printk(KERN_ALERT"[%s][%d]  i2c_write_buff_ds302 failed1\n",__func__,__LINE__);
			s32Ret =i2c_write_buff_ds302(i2c_connect_client,u8Data, 2);
			if( s32Ret <0 )
			{
				printk(KERN_ALERT"[%s][%d]  i2c_write_buff_ds302 failed2\n",__func__,__LINE__);
				return -1;
			}
		}
	}
...
int  enable_gt9xx_irq(void)
{
	...
	u8Data[0] = ICR_REG;   
	u8Data[1] = 0x21 ;
	s32Ret =i2c_write_buff_ds927(i2c_connect_client,u8Data, 2);
	if( s32Ret <0 )
	{
		printk(KERN_ALERT"[%s][%d]  i2c_write_buff_ds927 failed1 \n",__func__,__LINE__);
		s32Ret =i2c_write_buff_ds927(i2c_connect_client,u8Data, 2);
		if( s32Ret <0 )
		{
			printk(KERN_ALERT"[%s][%d]  i2c_write_buff_ds927 failed2 \n",__func__,__LINE__);
			return -1;
		}
	}	
	return 0;
...
{
	u8	u8Data[3]={0x00,0x00}; 
	mutex_lock(&gt9xxmutex);

	u8Data[0] = ISR_REG; 
	u8Data[2] = read_reg_ds927( i2c_connect_client,u8Data[0],2 );
	if( u8Data[2] < 0 )
	{
		printk(KERN_ALERT"[%s][%d]  clear_gt9xx_irq failed \n",__func__,__LINE__);
		mutex_unlock(&gt9xxmutex);
		return -1;
	}
	mutex_unlock(&gt9xxmutex);
...
static int ds927_i2c0_init(void) 
{
	s32 s32Ret = -1;
	u8	u8Data[3]={0x00,0x00};

	u8Data[0] = GENERAL_CONFIGURATION_REG; //0x03
	u8Data[2] = read_reg_ds927( i2c_connect_client,u8Data[0],2 );
	...
	u8Data[0] = GENERAL_CONFIGURATION_REG;	
	u8Data[2] = read_reg_ds927( i2c_connect_client,u8Data[0],2 );
	//DS_DBG("  ----read_reg_ds927--GENERAL_CONFIGURATION_REG----------u8Data[2] =0x%x \n",u8Data[2]);
	...
	u8Data[0] = DES_ID_REG;
	u8Data[1] = (GT9XX_I2C_DEVICE_ADDRESS<<1);
	s32Ret = i2c_write_buff_ds927(i2c_connect_client,u8Data, 2);
	if( s32Ret <0 )
	{
		printk(KERN_ALERT"[%s][%d]  i2c_write_buff_ds927 failed1 \n",__func__,__LINE__);
	...
	u8Data[0] = DS302_LOOP_THROUGH_REG;
	u8Data[1] = 0x00;
	s32Ret = i2c_write_buff_ds302(i2c_connect_client,u8Data, 2);
	if( s32Ret <0 )
	{
...
Output:
    None.
*********************************************************/
void gtp_irq_disable(struct goodix_ts_data *ts,int line)
{
...
Output:
    None.
*********************************************************/
void gtp_irq_enable(struct goodix_ts_data *ts,int line)
{
...
static void goodix_ts_work_func(struct work_struct *work)
{
...
#else
	#if 0//defined(LINUX_PLATFORM)
	u8 u8CurId[GTP_MAX_TOUCH] = {INVALID_ID, INVALID_ID, INVALID_ID, INVALID_ID, INVALID_ID};
	static u8 s_u8PreId[GTP_MAX_TOUCH] = {INVALID_ID, INVALID_ID, INVALID_ID, INVALID_ID, INVALID_ID};
	...
	for (i = 0; i < GTP_MAX_TOUCH; i++)	
	{
		if (s_u8PreId[i] != u8CurId[i] && s_u8PreId[i] != INVALID_ID) 
		{
			gtp_touch_up(ts, i);
		}
	}

	memcpy(s_u8PreId, u8CurId, GTP_MAX_TOUCH);
    ...
    #endif
        {
            GTP_DEBUG("Touch Release! pre_touch=%d ",pre_touch);
            gtp_touch_up(ts, 0);
        }
    }
	#endif
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    GTP_RST_OUTPUT(gtp_rst_gpio, 0);   // begin select I2C slave addr
    msleep(ms);                         // T2: > 10ms
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	 ...
	 */
	#if !defined(GTP_CONFIG_OF) || defined(LINUX_PLATFORM) 
...
//GTP_DEBUG("Config Groups\' Lengths: %d, %d, %d, %d, %d, %d\n", cfg_info_len[0], cfg_info_len[1], cfg_info_len[2], cfg_info_len[3],cfg_info_len[4],cfg_info_len[5]);//GTP_DEBUG
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        //TS_DBG("Drv * Sen: %d * %d(key: %d), X_MAX: %d, Y_MAX: %d, TRIGGER: 0x%02x  \n",
...
s32 gtp_read_version(struct i2c_client *client, u16* version)
{
    ...
    if (buf[5] == 0x00)
    {
        //TS_DBG("IC Version: %c%c%c_%02x%02x   \n", buf[2], buf[3], buf[4], buf[7], buf[6]);
		pr_info("[%d] IC Version: %c%c%c_%02x%02x \n", __LINE__,buf[2], buf[3], buf[4], buf[7], buf[6]);			
    }
    else
    {
        //TS_DBG("IC Version: %c%c%c%c_%02x%02x   \n", buf[2], buf[3], buf[4], buf[5], buf[7], buf[6]);
    }
    return ret;
}

s32 gtp_read_coor( struct i2c_client *client )
{
    s32 ret = -1;
    u8 buf[4] = {GTP_READ_COOR_ADDR >> 8, GTP_READ_COOR_ADDR & 0xff};
	
    ret = gtp_i2c_read(client, buf, sizeof(buf));
    if (ret < 0)
    {
        GTP_ERROR("GTP read  failed");
        return ret;
    }

  	printk(KERN_ALERT"[gt911][%s][%d]: 0x%02x  0x%02x  0x%02x  0x%02x \n",__func__,__LINE__,buf[0], buf[1],buf[2], buf[3] );
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    if (ret)
    {
        //GTP_ERROR("Request request_irq!ERRNO:%d.", ret);
		printk(KERN_ALERT"[%s][%d]  request_irq failed %d \n",__func__,__LINE__,ret);
    ...
    else 
    {
		...
		clear_gt9xx_irq();
        ts->use_irq = 1;
		pr_info("[%s][%d] ******** ts->use_irq: 0x%x \n",__func__, __LINE__,ts->use_irq);		
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (ret < 0)
    {
		...
		{
			errcode_repo(ENUM_HAL_CODE_TOUCHSCREEN,ENUM_HAL_VALUE_TOUCHSCREEN_INIT_DSP927_FAIL);   //存储启动之后的错误码
			printk(KERN_EMERG "LINE = %d FUNC = %s repo a code ！！！\n",__LINE__, __func__);
		}
        return -1;
	}
    ...
    if (ret < 0)
    {
		...
		if(ERRCODE_FLAG == on_bootz)
		{
			printk(KERN_EMERG "LINE = %d FUNC =%s [BSP]----DO NOT SAVE CODE！！！\n",__LINE__, __func__); // 暂时不存储启动的错误码，以log代替
		}
		if(ERRCODE_FLAG == after_bootz)
		{
			errcode_repo(ENUM_HAL_CODE_TOUCHSCREEN,ENUM_HAL_VALUE_TOUCHSCREEN_INIT_FAIL);   //存储启动之后的错误码
			printk(KERN_EMERG "LINE = %d FUNC = %s repo a code ！！！\n",__LINE__, __func__);
		}
    }
    ...
    gt9xx_enable_irq = proc_create("ts_ops", 0666, NULL, &enable_proc_ops);
