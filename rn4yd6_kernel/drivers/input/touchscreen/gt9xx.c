...
static ssize_t gt91xx_enable_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
	...
	recv[2] ='\0';	
    printk(KERN_EMERG "strlen(msg) = %d\n", strlen(recv));
	printk(KERN_EMERG "write str is (%s)\n", recv);
	...
	if(strcmp(recv,"01") == 0 )
	{
		...
		ret = gtp_i2c_write(ts_all->client, point_data, 2 + 1 + 8 * GTP_MAX_TOUCH + 1);	
