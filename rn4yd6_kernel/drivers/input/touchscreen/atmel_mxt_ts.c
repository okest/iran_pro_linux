...
static int mxt_initialize(struct mxt_data *data)
{
	...
	while (1) {
		...
		LOGD("read the info end error = %d",error);
		if (!error)
			break;
...
#if defined(CONFIG_FORYOU_CSR)
static int mxt_ResetChip(unsigned long u32Gpio)
{
	if (gpio_is_valid(u32Gpio))
	{
		if (0 == gpio_request(u32Gpio, "atmel,reset-gpio")) {
			pr_info("[%s] Line:%d pull down for %d ms.\r\n", __FUNCTION__, __LINE__, MXT_RESET_TIME);
			gpio_direction_output(u32Gpio, 0);
			msleep(MXT_RESET_TIME);
			gpio_direction_output(u32Gpio, 1);
			msleep(MXT_RESET_TIME);
	...
	pr_err("[%s] Line:%d gpio_no:%d reset fail!\r\n", __FUNCTION__, __LINE__, u32Gpio);
	return -1;
}
#endif
