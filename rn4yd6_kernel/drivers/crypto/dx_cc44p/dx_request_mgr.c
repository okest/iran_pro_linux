	...
	HwDesc_s seq[], unsigned int seq_len)
{
	...
	for (i = 0; i < seq_len; i++) {
		writel_relaxed(seq[i].word[0],
			(void __iomem *)(cc_base +
			DX_CC_REG_OFFSET(CRY_KERNEL, DSCRPTR_QUEUE0_WORD0)));
		writel_relaxed(seq[i].word[1],
			(void __iomem *)(cc_base +
			DX_CC_REG_OFFSET(CRY_KERNEL, DSCRPTR_QUEUE0_WORD0)));
		writel_relaxed(seq[i].word[2],
			(void __iomem *)(cc_base +
			DX_CC_REG_OFFSET(CRY_KERNEL, DSCRPTR_QUEUE0_WORD0)));
		writel_relaxed(seq[i].word[3],
			(void __iomem *)(cc_base +
			DX_CC_REG_OFFSET(CRY_KERNEL, DSCRPTR_QUEUE0_WORD0)));
		writel(seq[i].word[4],
			(void __iomem *)(cc_base +
...
static void proc_completions(struct dx_drvdata *drvdata)
{
	...
	while(request_mgr_handle->axi_completed) {
		...
		if (rc != 0) {
			DX_LOG_ERR("Failed to set runtime suspension %d\n",rc);
		}
#endif
	}
}
