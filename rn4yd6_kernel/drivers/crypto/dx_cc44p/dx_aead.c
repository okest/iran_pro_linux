	...
	unsigned int *seq_size)
{
	...
	case DX_DMA_BUF_MLLI:
		...
		HW_DESC_SET_DIN_TYPE(&desc[idx], DMA_MLLI,
			areq_ctx->assoc.sram_addr, 0, AXI_ID, NS_BIT);
		HW_DESC_SET_FLOW_MODE(&desc[idx], flow_mode);
		if (ctx->auth_mode == SEP_HASH_XCBC_MAC)
			HW_DESC_SET_DIN_NOT_LAST_INDICATION(&desc[idx]);
		break;
	...
	int direct)
{
	...
	case DX_DMA_BUF_MLLI:
		...
		HW_DESC_INIT(&desc[idx]);
		HW_DESC_SET_DIN_TYPE(&desc[idx], DMA_MLLI,
			areq_ctx->src.sram_addr, 0, AXI_ID, NS_BIT);
		HW_DESC_SET_DOUT_MLLI(&desc[idx],
			areq_ctx->dst.sram_addr, 0, AXI_ID, NS_BIT, 0);
		HW_DESC_SET_FLOW_MODE(&desc[idx], flow_mode);
		break;
	case DX_DMA_BUF_NULL:
	default:
	...
	unsigned int data_flow_mode)
{
	...
	dx_aead_process_cipher_data_desc(req, data_flow_mode, desc, &idx, direct);
	if (direct == SEP_CRYPTO_DIRECTION_ENCRYPT) {
	...
	unsigned int *seq_size)
{
	...
	if (req_ctx->is_single_pass == true) {
		...
		dx_aead_process_cipher_data_desc(req, data_flow_mode, desc, seq_size, direct);
...
/* Remove #ifdef when CC products will adapt
*  same single-pass restrictions. */
#if (DX_DEV_SIGNATURE == DX_CC442P_LT_SIG)
...
#else /*DX_CC442P_SIG*/
		if ((ctx->cipher_mode == SEP_CIPHER_CTR) ||
		    !IS_ALIGNED(req->assoclen, sizeof(uint32_t)))
			areq_ctx->is_single_pass = false;
#endif
		break;
	case S_DIN_to_DES:
	...
	unsigned int *seq_size)
{
	...
	if (req->assoclen > 0) {
		dx_aead_create_assoc_desc(req, DIN_AES_DOUT, desc, &idx);
	} else {
		HW_DESC_INIT(&desc[idx]);
		HW_DESC_SET_DIN_TYPE(&desc[idx], DMA_DLLI, 
	...
	idx++;
	if (req_ctx->cryptlen != 0) {
		dx_aead_process_cipher_data_desc(req, DIN_AES_DOUT, desc, &idx, direct);
	}
