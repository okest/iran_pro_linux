...
static int init_cc_resources(struct platform_device *plat_dev)
{
	...
	/* Enable clock before all operations */
	new_drvdata->clk = devm_clk_get(&plat_dev->dev, NULL);
	if (IS_ERR(new_drvdata->clk)) {
		DX_LOG_ERR("Failed to get ds-cc-pub clock\n");
		rc = -EPROBE_DEFER;
		goto init_cc_res_err;
	}
	...
	if (rc) {
		DX_LOG_ERR("Enable ds-cc-pub clock failed\n");
		goto init_cc_res_err;
	}
...
#warning Not an ARM(32) platform - Using default DMA operations
	...
	DX_LOG(KERN_INFO, "CryptoCell CC442P Driver: ROM version 0x%08X\n", rom_ver);
#endif
...
init_cc_res_err:
	...
	if (new_drvdata != NULL) {
		...
		clk_disable(new_drvdata->clk);
		clk_unprepare(new_drvdata->clk);
		clk_put(new_drvdata->clk);
