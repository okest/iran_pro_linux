...
#define SD_DBG_ENABLE     1
#if SD_DBG_ENABLE
#define SD_DBG(fmt, args...)  printk(KERN_ALERT"\n[%s][%d] " fmt, __func__, __LINE__, ## args)
#else
#define SD_DBG(fmt, args...)
#endif
...
static void sdhci_data_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (!host->data && !priv->loopdma) {
		...
		pr_info("%s: Got data interrupt 0x%08x even ""though no data operation was in progress.\n",mmc_hostname(host->mmc), (unsigned)intmask);
...
struct sdhci_host *sdhci_host_g ;
int g_flag;
void sdhci_bus_scan(int flag)
{
	//printk(KERN_ALERT"[%s][%d]==============flag=%d \n", __FUNCTION__,__LINE__,flag);
	...
	if(flag)
	{
		sdhci_card_event(sdhci_host_g->mmc);
